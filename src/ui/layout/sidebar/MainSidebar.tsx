...
import {
  DRAWER_CLOSE_WIDTH_LG,
  DRAWER_OPEN_WIDTH,
} from "@src/ui-shared/constants/Constants";
...
export const MainSidebar: FC<Prop> = ({ toggleDrawer, open }) => {
  ...
  const renderMenuSectionLabel = (
    ...
    if (
      ...
      return (
        <StyledListSubheader color="primary">
          {sectionLabel}
        </StyledListSubheader>
      );
    }
  ...
  const displayMenu = Object.values(ROUTES).map(
    ({ path, label, sectionLabel, icon, hideOnSideMenu }, index) => {
      if (hideOnSideMenu) return;
      const prevSectionLabel = ROUTES[index - 1]
        ? ROUTES[index - 1].sectionLabel
        : undefined;
      return (
        <Grid key={path}>
          <Tooltip
